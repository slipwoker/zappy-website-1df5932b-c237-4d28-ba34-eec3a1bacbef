  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      if (!fullName || !email || !phone || !terms) {
        alert('אנא מלא את כל השדות החובה ואשר את תנאי השימוש');
        return;
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "א.א.ת איתור נזילות משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '1df5932b-c237-4d28-ba34-eec3a1bacbef',
                    name: data.name || '',
                    email: data.email || '',
